...
export class InventoryConstService {
  ...
  public static factoryArbreAttribute(arbre: Arbre) {
    arbre.caractPied = arbre.caractPied.length !== 0 ? arbre.caractPied.filter(data => data != 'other') : [];
    ...
    arbre.critere = (arbre.critere.length !== 0) ? arbre.critere.filter(data => data != 'autre-precisez') : [];
    arbre.critere = InventoryConstService.factoryArray(arbre.critere, inventoryConst.CRITERE_TAB);

    arbre.proximite = (arbre.proximite.length !== 0) ? arbre.proximite.filter(data => data != 'other') : [];
    ...
    // ETAT_SANITAIRE
    arbre.etatSanCollet = arbre.etatSanCollet.length !== 0 ? arbre.etatSanCollet.filter(data => (data != 'champignons-lignivores' && data != 'cavit-nombre' && data != 'other')) : [];
    arbre.etatSanCollet = InventoryConstService.factoryArray(arbre.etatSanCollet, inventoryConst.ETAT_SAN_COLLET_TAB);

    arbre.etatSanTronc = arbre.etatSanTronc.length !== 0 ? arbre.etatSanTronc.filter(data => (data != 'champignons-lignivores' && data != 'cavit-nombre' && data != 'parasite' && data != 'corps-etranger' && data != 'parasite-other' && data != 'other')) : [];

    arbre.etatSanTronc = InventoryConstService.factoryArray(arbre.etatSanTronc, inventoryConst.ETAT_SAN_TRONC_TAB);

    arbre.etatSanHouppier = arbre.etatSanHouppier.length !== 0 ? arbre.etatSanHouppier.filter(data => (data != 'champignons-lignivores' && data != 'parasite' && data != 'other')) : [];

    arbre.etatSanHouppier = InventoryConstService.factoryArray(arbre.etatSanHouppier, inventoryConst.ETAT_SAN_HOUPPIER_TAB);

    arbre.etatSanGeneral = arbre.etatSanGeneral.length !== 0 ? arbre.etatSanGeneral.filter(data => data != 'other') : [];
    ...
    arbre.typePassage = arbre.typePassage.length !== 0 ? arbre.typePassage.filter(data => data != 'other') : [];
  ...
  static factoryEssenceAttribute(essence: Essence): any {
    ...
    essence.proximite = essence.proximite.length !== 0 ? essence.proximite.filter(data => data !== 'other') : [];
    ...
    // CRITERE
    essence.critere = essence.critere.length !== 0 ? essence.critere.filter(data => data !== 'comment') : [];
    ...
    // TYPE PASSAGE
    essence.typePassage = essence.typePassage.length !== 0 ? essence.typePassage.filter(data => data !== 'other') : [];
    ...
    // ETAT GENERAL
    essence.etatGeneral = essence.etatGeneral.length !== 0 ? essence.etatGeneral.filter(data => (data !== 'comment' && data !== 'champignons-lignivores' && data !== 'parasite' && data !== 'parasite-other')) : [];
    ...
    // TRAVAUX
    essence.travaux = essence.travaux.length !== 0 ? essence.travaux.filter(data => (data !== 'other' && data !== 'soin-particulier-precisez' && data !== 'protection-particuliere-precisez')) : [];
