...
export class HomeComponent implements OnInit {
  ...
  getDict(momentSelected = 'DEFAULT') {
    let endMonthDate,
      startMonthDate;
    const monthEnCourt = this.getSelectedMoment(momentSelected);
    ...
    if (momentSelected === 'CURRENT') {
      endMonthDate = monthEnCourt.clone().endOf('month');
      startMonthDate = monthEnCourt.clone().startOf('month');
      this.countStaticDict(startMonthDate, endMonthDate);
  ...
  private countStaticDict(startMonthDate, endMonthDate) {
    let DICT = 0;
    if (startMonthDate == null && endMonthDate == null) {
      this.searchedDatas.forEach((inv: Inventaire) => {
        if (inv.arbre && inv.arbre.proximiteWithDict.length > 0) {
          DICT++;
        } else if (inv.epaysage && inv.epaysage.essence.length > 0) {
          inv.epaysage.essence.forEach((essence: Essence) => {
            if (essence.proximiteWithDict && essence.proximiteWithDict.length > 0) DICT++;
          })
        }
      });
    } else {
  ...
  personalValid() {
    ...
    if (this.toDate !== null) {
      ...
      if (this.blocIdentity === 'BLOC_PIE_CHART') {
        this.countprintPieChartStatic(startMonthDate, endMonthDate)
      } else if (this.blocIdentity === 'BLOC_DICT') {
        this.countStaticDict(startMonthDate, endMonthDate);
