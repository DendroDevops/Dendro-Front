...
interface customProfilModele extends ProfilModele{
  nameDroit: string
}
@Component({
  selector: 'app-profil',
...
export class ProfilComponent implements OnInit {
  ...
  columns: ColumnInterface[] = [
    {name: 'name', display: 'Profil', style: {"min-width": "18rem"}, isModelProperty: true, isVisible: true, isSort: true, isString: true},
    {
      name: 'nameDroit',
      display: 'Droit',
    ...
    {name: 'createdAt', isDate: true, display: 'Date création', isModelProperty: true, isVisible: true, isSort: true},
  ...
  lists() {
    ...
    this.profilService.list()
      .subscribe((data: ProfilModele[]) => {
        ...
        this.profils = data.map(elm => {
          return {...elm, nameDroit: elm.droit.name}
        });
